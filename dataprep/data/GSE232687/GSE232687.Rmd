...
saveRDS(dge,file="GSE232687.Rds")
...
y <- read.table("GSE231784_quickmirseq.mapped.mirna.filtered.length.GEO.csv.gz",sep=",",header=TRUE)
head(y)
y$length = y$Seq = NULL
ya <- aggregate(. ~ miRNA, y, sum)
rownames(ya) <- ya$miRNA
ya$miRNA = NULL

yaf <- ya[which(rowMeans(ya)>10),]
dim(ya) ; dim(yaf)


ssy <- data.frame("run"=colnames(ya),"trt"=c(0,1,0,1,0,1))
rownames(ssy) <- colnames(ya)

colSums(yaf)

dds <- DESeqDataSetFromMatrix(countData = round(yaf) , colData = ssy , design = ~ trt )
res <- DESeq(dds)
z <- results(res)
...
saveRDS(dge2,file="GSE232687_mirna.Rds")
